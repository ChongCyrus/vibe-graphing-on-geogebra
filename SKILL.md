...
如果模型输出不是 JSON 对象、也没有 ggb/js 代码块，应用会返回“格式错误”工具结果并要求重试；
连续 3 次格式错误会自动停止。模型必须把要执行的命令放在 `code` 字段中，
`thinking` 字段不会被解释执行。

工具结果会自动回传给模型，包括：每条命令是否成功、错误信息、当前作图快照。
快照包含**全部对象**的完整属性（可见性、颜色、填充、线型线宽、点型点大小、
标签、图层、固定、跟踪、定义、值、坐标）和视图状态（坐标范围、坐标轴、
网格、背景色等）；首轮还会额外收到一次初始作图状态。模型输出以 SSE 流式传输，
用户可实时查看思考链与回答；点击聊天窗口顶部的“思考链”条可显示/隐藏思考链。
会话会持久化保存在本地，支持新建/切换/重命名/删除会话，首个会话标题由模型自动生成。
